...
enum ForecastMiddleware {
    ...
    static func executeGetWeatherInfo() -> Middleware<ForecastState, ForecastAction> {
        { _, action in
    ...
    static func executeGetWeatherActiveInfo() -> Middleware<ForecastState, ForecastAction> {
        { _, action in
...
extension ForecastMiddleware {
    private static func log(error: Error, dispatch: ForecastAction) -> AnyPublisher<ForecastAction, Never> {
